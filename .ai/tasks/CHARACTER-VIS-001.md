...
- **V-1 第2段階の Human Decision**: ユーザー（人間）/ 2026-09-26 / 会話で回答
  1. 上位4職の酒場での撮影は「不要」（トレーニング空間の撮影で可。P-D11 の酒場撮影の必須は影の旅人に適用）
  2. 影の旅人のコート・パンツは「A でよい」（承認値 #30323A / #403454 のまま。明るめの B #45474F / #4C3F63、C #55575F / #56487A は不採用）
...
- **V-1a Human OK**: ユーザー（人間）/ 2026-09-26 / 会話で、V-1a の比較画像（武器の拡大・低いカメラ・標準カメラ・生成時 / 持ち替え後）を見て「V-1a Human OKをTaskへ記録し、V-1bだけ実施」と指示。武器装飾の trim 色（盗賊の双剣の暗色化 RF-3、弓師・鷹の目の Off White を含む）・武器の質感・ジェムの発光・生成時と持ち替え後のルールの一致を Human 確認済みとする（AC-10）
...
### V-1b（上位職の装飾と武器の輪郭線）
- 撮影: V-1a と同じ撮影（`f8bcdb6` の内容。以降の差分は Task file のみ）から、上位4職の装飾・オフハンド・持ち替えた武器を拡大して T-6 前（`main` `7bce8db`）と並べた
- 事実（画像）:
  - 戦騎士（強化肩・ハーネス・白いジャケット・袖・ベルト）、魔導士（上掛け・白いドレス・袖口・長い銀髪・房飾り）、鷹の目（背中のフード・ベスト・ポケット・肩の鷹）、バーサーカー（半袖の上着・袖）: T-6 前は輪郭線が無く、T-7 では本体と同じ暗い線 + クリーム色の縁取りが付いている
  - 魔導士の浮遊する結晶・魔法陣、バーサーカーの足元のオーラ: T-7 でも輪郭線は付かない（P-D4 の対象外）
  - オフハンド（盗賊の双剣・バーサーカーの両手斧）: 生成時の武器は T-6 前から本体と一緒に輪郭線がある（変化なし）
  - 持ち替えた武器（剣士、酒場、ちぞめの大剣）: 輪郭線あり。特殊武器のオーラには付かない
- 画像で確認できないもの（FACT）: X 線シェルは壁などに隠れたときだけ描かれるため、この撮影では見えない（構造は E2E の OUTL 行で欠け 0 を確認）。**持ち替えたオフハンド**は、本編で盗賊系の主人公が特殊武器を装備する導線を撮影・E2E できないため未確認（コードは主武器と同じ `addPartOutlines([weapon, P.offhandWeapon])`、unit のソース検査で確認）
- **V-1b Human OK**: ユーザー（人間）/ 2026-09-26 / 会話で V-1b の比較画像（上位4職の装飾・オフハンド・持ち替えた武器）を見て「okです」。上位職の装飾・武器の輪郭線の見え方と、画像で確認できない点（X 線シェル、持ち替えたオフハンド）の扱いを Human 確認済みとする（AC-11）

### Out of Scope Found（T-7 の V-1b で Human が指摘、未対応）
- **手元の浮いた丸い粒**（Human: 「手元の丸い粒みたいな浮いているグラフィックは指かな？意味がなければ消して欲しい」）
...
- V-1c（手元）: 8職の低いカメラ・攻撃入力 1.2 秒後の手元を拡大して修正前（`f8bcdb6`）と比較。修正前は弓師・盗賊・バーサーカーなどで手の横に浮いた丸い粒が写り、修正後は写らない（指が手に付いている）。**V-1c Human OK**: ユーザー（人間）/ 2026-09-26 / 会話で手元の比較画像を見て「OK」
- `docs/CHARACTERS.md`: 外見の節に武器の装飾色・質感・輪郭線の記述を追加（V-1 OK 後、Files To Change どおり）
