...
// into one shared scope at build time; see src/legacy/concat-plugin.js)
  ...
  function applySaveData(data){
    ...
    /* 旧セーブには無いので、鍛冶士が加入済みなら回収済みとみなす ――
       洋館をクリアした以上、工具は持ち帰っているはずだから */
    state.smithToolsRecovered = data.smithToolsRecovered === undefined
      ? !!data.smithJoined : !!data.smithToolsRecovered;
    // 同行/分離と空間異常は出撃中だけの状態。ロード時は必ず初期値へ戻す
  ...
  window.addEventListener('keydown', e=>{
    ...
    // Visual Freeze。toggleMotionFreeze() 自身が state.debugMode で弾くので、
    // 通常プレイで P を押しても何も起こらない(05-rendering-rig.js)
    if(e.code==='KeyP' && !e.repeat){ toggleMotionFreeze(); }
