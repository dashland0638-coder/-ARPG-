...
// into one shared scope at build time; see src/legacy/concat-plugin.js)
  ...
  const SCENARIO_TAVERN_DIALOGUE = {
    /* 最初のシナリオの導入。ここで怪異の正体を説明してしまわないこと。
       主人も断定はせず、噂と、確かめられる事実だけを渡す */
    mansion: ()=>{ const M='酒場の主人', Y=state.name||'あなた'; return [
      {name:M, text:'……見慣れぬ顔だな。旅の冒険者かい。'},
      {name:Y, text:'この辺りで、何か腕試しになりそうな話はないか?'},
      {name:M, text:'あるにはある。森の奥に古い洋館があってな。半月ほど前から、あそこへ行った者が戻ってこん。'},
      {name:Y, text:'戻ってこない?'},
      {name:M, text:'ああ。荷を運んだ男も、工具を取りに行った鍛冶屋も。……鍛冶屋のほうは、まだ二日だがね。'},
      {name:Y, text:'何がいるんだ。'},
      {name:M, text:'知らんよ。誰も見たと言い切らん。人影を見たと言う者、声を聞いたと言う者、気のせいだと笑う者。全員が本気で言うんだ。'},
      {name:Y, text:'……なるほど。'},
      {name:M, text:'道は一本だ。迷いはせん。……帰ってこられるなら、その話をぜひ聞かせてくれ。'}
    ]; },
  ...
  const SHADOW_GUIDE_FIRST_MEET = ()=>{ const S=SHADOW_GUIDE_NAME, Y=state.name||'あなた'; return [
    ...
    /* 鍛冶士は洋館から連れ帰るまで酒場に居ない(state.smithJoined)。
  ...
  function showBossResultScreen(boss, levelBefore){
    ...
    /* 酒場が変わる最初のきっかけ。洋館から戻ると、屋敷で出会った鍛冶士が
       酒場の一角を間借りして店を開く。以降 buildTavern() が本人と炉を建てる
       (それまでは同じ場所に仮設の作業台が置いてある) */
    if(scKey === 'mansion' && !state.smithJoined) state.smithJoined = true;
  ...
  const BOSS_ENDING_LINES = {
    mansionBoss: [
      '椅子の上の輪郭が、一瞬だけ、ちゃんとした人のかたちに見えた。',
      '男は少し驚いたような顔をして――それから、ひどく安心したように息を吐いた。',
      '「……ああ。そうか」――それだけ言って、上着だけが椅子に残った。',
      '窓の外は、来た時と同じ森だ。何があったのかは、結局わからないままだった。'
    ],
    ghostCaptain: [
      '「港が……見える……」――船長が最後に、そう呟いた。',
  ...
  const SCENARIO_DEFS = [
    ...
    {key:'mansion',    name:'🏚️ 囚われの洋館',   levelRange:'1〜30(★8)',   minLevel:1,  desc:'森の一本道を抜けた先に佇む古い洋館。訪れた者が続けて戻らなくなった。何がいるのかは、誰も言い切れない。', unlocked:true},
