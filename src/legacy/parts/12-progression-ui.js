...
// into one shared scope at build time; see src/legacy/concat-plugin.js)
  ...
  const BOSS_DIALOGUE_DEFAULT = [
    '……侵入者か。この館に、何用だ。',
    'かつて我は、この館の主であった。愛した家族を喪い、禁忌の呪術に手を染めた……',
    '蘇ったのは、あれの記憶を纏っただけの何かだった。それでも、我は術を止めなかった。',
    '気づけば、家族だけでなく、この館そのものが呪いに呑まれていた――もはや、後戻りはできぬ!'
  ];
  ...
  const SCENARIO_TAVERN_DIALOGUE = {
    mansion: ()=>{ const M='酒場の主人', Y=state.name||'あなた'; return [
      ...
      {name:Y, text:'どんな悲劇だ?'},
      {name:M, text:'当主アルベルト卿は、亡くした家族を蘇らせようとして、手を出しちゃならんものに手を出したそうだ。詳しいことは誰も知らんがね。'},
  ...
  const BOSS_ENDING_LINES = {
    mansionBoss: [
      '崩れゆく静寂の中、当主アルベルトの姿が薄れていく。',
      '暖炉の向こう、幻の食卓に、家族の姿が一瞬だけ戻った。',
      'アルベルトも席につく。「……ただいま。」',
      '暗転。屋敷を後にする足取りは、来た時よりも幾分か軽かった。'
    ],
    ghostCaptain: [
      '「港が……見える……」――船長が最後に、そう呟いた。',
  ...
  // 各ダンジョンの既存ロア(館主アルベルト=禁忌の呪術で家族を蘇らせようと
  // した執着/帰港を望む船長=呪われた錨を手放せない/水路の主=夢とも知れぬ
  ...
  const BOSS_BARK_LINES = {
    mansionBoss: {
      hi: '「まだだ……まだ、あれを取り戻せるはずだ……」',
      lo: '「頼む……もう一度だけ……家族に、会わせてくれ……」'
    },
    ghostCaptain: {
